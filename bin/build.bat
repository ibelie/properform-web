...
MD %GOOGPATH%
COPY %CLOSURE%\goog\base.js %GOOGPATH%\
COPY %TSPATH%\lib\jquery\jquery-3.2.1.min.js %JSPATH%\jquery.js
IF defined DEBUG (
	CD %JSPATH%
	CALL python -B %CLOSURE%\build\depswriter.py --root_with_prefix=". ../js" > ..\goog\deps.js
) ELSE (
	CALL python -B %CLOSURE%\build\closurebuilder.py --root=%CLOSURE%/ --root=%JSPATH%/ ^
		--namespace="HelloWorld" ^
		--output_mode=compiled ^
		--compiler_jar=%CLOSURE%\compiler.jar ^
		--compiler_flags="--js=%CLOSURE%/goog/deps.js" ^
